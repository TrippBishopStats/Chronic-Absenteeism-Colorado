...
#### Districts with at least 1 suspicious `total_students` record
...
```
The changing rates of absenteeism show a trend of generally increasing rates peaking in the 2021-2022 school year
...
### Is district enrollment flux correlated with chronic absenteeism?
...
df_master_tmp <- df_master |> 
  mutate(
    pct_enroll_diff = if_else(abs(pct_enroll_diff) > 20, NA, pct_enroll_diff)
  )
  
districts_shp |> 
  inner_join(df_master_tmp, by="district_code") |>
  filter(school_year != "2016-2017") |> 
  ggplot() +
  geom_sf(aes(fill=pct_enroll_diff)) +
  facet_wrap(~school_year, ncol = 2) +
  scale_fill_gradient2(low="#557ca2", high="#b25e5a", 
                       mid="white", midpoint = 0
  ) +
  labs(
    fill="% change",
    title = "Percent change in student enrollment by district, 2017-2023",
    subtitle = "Grey indicates absolute changes > 20%"
  ) +
  theme(
    plot.title.position = "plot",
    legend.position = "bottom",
